...
import { adminLogOut, adminUserFetchAction } from "../../../Redux/Actions/adminActions";
...
<Button icon="pi pi-user" rounded severity="info" aria-label="User" onClick={OnAdminLogOut} />
      ...
      <DataTable value={users} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}>
        ...
        <Column
          body={(rowData) => (
            <Button label="unBlock" severity="success" outlined className="my-button" onClick={() => handleViewClick(rowData._id)} />
          )}
        />
        <Column
          body={(rowData) => (
            <Button label="Block" severity="danger" outlined className="my-button" onClick={() => handleViewClick(rowData._id)} />
          )}
        />
